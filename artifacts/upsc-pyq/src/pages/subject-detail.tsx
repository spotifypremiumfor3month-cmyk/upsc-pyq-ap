...
import { ChevronLeft, Search, Filter, CheckCircle2, ChevronRight, PlayCircle, Lock } from 'lucide-react';
...
export default function SubjectDetail() {
  ...
  const handleStartTest = (e: React.MouseEvent) => {
    ...
    // if logged in, Link href handles navigation normally
  };
          ...
          <>
            ...
            <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm bg-card border rounded-2xl shadow-2xl z-[101] p-8 text-center space-y-5 animate-in zoom-in-95 duration-200">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 px-5 py-3 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-sm shadow-md transition-all active:scale-95"
              >
              ...
              <button onClick={() => setShowLoginPrompt(false)} className="text-xs text-muted-foreground hover:text-foreground transition-colors">Cancel</button>
            </div>
          </>
        )}
      </div>
